...
def cadastrar_peca():
    if request.method == 'POST':
        ...
        if not nome or not fabricante or not descricao or not valor or not image_file:
        ...
        peca = Peca(nome=nome, fabricante=fabricante, descricao=descricao, valor=valor, image_url=image_filename)
        db.session.add(peca)
